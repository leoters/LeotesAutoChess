...
function validateBoard(board, stage) {
    const team = board.map((p) => (0, precomputed_pokemon_data_1.getPokemonData)(p.name)).filter(p => p.passive !== Passive_1.Passive.INANIMATE);
    ...
    const numberOfGoldBows = board.flatMap((p) => p.items).filter((item) => item === Item_1.Item.GOLD_BOW).length;
    ...
    function removeUniqueThatEvolveToLegendary(p) {
        if ((0, precomputed_pokemon_data_1.getPokemonData)(Pokemon_1.PkmFamily[p.name]).rarity === Game_1.Rarity.UNIQUE && (0, precomputed_pokemon_data_1.getPokemonData)(p.name).rarity === Game_1.Rarity.LEGENDARY) {
            return false;
        }
        return true;
    }
